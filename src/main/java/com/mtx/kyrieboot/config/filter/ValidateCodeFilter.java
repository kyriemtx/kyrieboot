...
public class ValidateCodeFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (CommonConstants.REQUEST_MODE_POST.equals(request.getMethod()) && PATHMATCHER.match(CommonConstants.LOGIN_URL,request.getServletPath())){
            try {
    ...
    private void codeValidate(HttpServletRequest request) {
        ...
        if (StringUtils.isEmpty(code)){
            throw new BizException("验证码的值不能为空");
        }
